...
- **Data-dependent footprints — PROMOTED (2026-07-11), then CONFIRMED and FIXED
  ...
  is flagged, so the scheduler trusts a footprint that names the wrong ids — and an
  inaccurate declared footprint is unsound for READS by exactly the H3 argument,
  however it got that way. The H3 fix cannot reach it precisely because nothing
  threw.
