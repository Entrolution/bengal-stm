...
**H2 FIXED (2026-07-11)**: `TxnLogEntry.lock` now carries each lock's OWNER id
and `withLock` sorts on that, which is a genuine total order over locks;
`CommitH2.cfg` flipped to expected-clean and `CommitLockOrderSpec` pins it
behaviourally. **H3 remains OPEN** (pinned red), fix pre-validated by NC-2.
