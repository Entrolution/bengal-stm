...
| `CommitSnapshotValid` | `src/main/scala/bengal/stm/model/runtime/IdFootprint.scala` | `isUnderApproximated` + the first clause of `isCompatibleWith`: a footprint the walker could not complete is incompatible with everything, so the transaction is serialized and runs alone — the H3 fix. HOLDS (the anchor moved here from the pre-fix mechanism site, per the convention above) |
| `CommitSnapshotValid` | `src/main/scala/bengal/stm/runtime/TxnRuntimeContext.scala` | `TxnRuntime.commit`'s `handleErrorWith` marks **both** of its under-approximating branches (the partial footprint and the empty one) — the flag is only sound if every producer sets it |
