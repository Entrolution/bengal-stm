...
| `CoverageSubsumesDirty` — the deleted commit-time dirty check could never fire | **HOLDS** on every config — the machine-checked justification for deleting the dirty check: if the declared footprint covers the actual one, the write set cannot have moved underneath the transaction (a peer publishing to my write set must have declared that write; co-writers are incompatible; Contract C kept it out of my window). Asserted as an invariant by all seven configs; the NC-8 discussion below explains why it is pinned as an invariant rather than a mutation control | all `commit/` cfgs |
...
declares it, and `grep -rn NoLostWakeup specs/` returns only prose (this file and a
`Scheduler.tla` comment). A lost
...
| `writeSeq[t]` | the lock-contributing (update) entries ordered by their **LOCK'S OWNER** id — `SortedWrites`/`LockOwnerVal` in the model, `locks.flatten.distinct.sortBy(_._1.value)` in `withLock`, where the `_._1` is the owner's runtime id each entry's lock is paired with. That owner ordering **is** the H2 fix; sorting by the log key instead is negative control NC-1 |
