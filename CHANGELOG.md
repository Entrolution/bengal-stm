...
- JMH throughput benchmarks (`benchmarks/`, not aggregated into the root project): seven, covering the fixes that carry an obvious price — H3's serialization cliff (−41%), H5's whole-map-reader-versus-inserter conflict, H6's coverage check on every commit, and the lock path H2 rewrote. Nothing here isolates H1, H4 or the self-wake spin
