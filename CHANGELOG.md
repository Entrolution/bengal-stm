...
### Changed
- `TxnLogValid.idFootprint` now folds its entries with `traverse` rather than `parTraverse`. Each entry's footprint is a pure computation, so there is nothing to overlap and a fiber per entry was pure overhead — and a whole-map read expands into one log entry per key, so the fiber count tracked the map size. Measured on dedicated hardware: +19% on whole-map-read workloads.

### Added
- TLA+ specification of the **commit protocol** (`specs/commit/CommitProtocol.tla`, "Spec A"): the log run, lock resolution and acquisition, the commit-time dirty check, publish, release and the dirty-refinement path, verified under an assumed Contract C that the scheduler spec proves. Ten CI-pinned TLC expectations across the workstream, all expected-clean
...
- JMH throughput benchmarks (`benchmarks/`, not aggregated into the root project) measuring the cost of every fix in this release
